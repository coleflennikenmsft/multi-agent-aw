...
class ReviewerExecutor(Executor):
    ...
    - If MAX_REVIEW_ITERATIONS is exceeded, the workflow ends with an error to
        ...
        if "IMPLEMENTATION INCOMPLETE" in review_text:
            if self._iteration >= MAX_REVIEW_ITERATIONS:
                print(
                    ...
                    "Ending workflow without approval."
                )
                await ctx.yield_output(
                    f"WORKFLOW HALTED: The implementation was not approved after "
                    f"{MAX_REVIEW_ITERATIONS} attempts.\n\nLast reviewer feedback:\n\n"
                    + review_text
                )
            else:
