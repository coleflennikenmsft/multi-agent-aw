...
class ReviewerExecutor(Executor):
    ...
    - If the implementation is approved, forwards to the tester via ctx.send_message.
    """
...
class TesterExecutor(Executor):
    """Tests the implementation and validates it works correctly.

    - If tests fail, sends the feedback back to the implementer (looping) via ctx.send_message.
    ...
    @handler
    async def handle(self, message: str, ctx: WorkflowContext[str, str]) -> None:
        print("[Tester] Running tests on implementation...")
        ...
        if "TESTS FAILED" in test_text:
            ...
            await ctx.send_message(
                "The tester found test failures. Please address the following "
                "and fix the implementation:\n\n" + test_text
            )
        else:
            print("[Tester] All tests passed — implementation validated.")
...
def build_workflow(client: CopilotClient):
    ...
    The workflow DAG is:
        planner → implementer → reviewer → tester
                       ↑            |           |
                       └────────────┼───────────┘  (loop when reviewer finds issues or tests fail)
    """
    ...
    workflow = (
        ...
        .add_edge(reviewer, tester)        # proceed to testing when approved
