...
class TesterExecutor(Executor):
    ...
    - If tests pass, forwards to the reviewer.
    """
    ...
    async def handle(self, message: str, ctx: WorkflowContext[str]) -> None:
        ...
        if "TESTS FAILED" in test_text:
            ...
            await ctx.send_message(
                "The tester found failures. Please fix the issues and re-implement:\n\n"
                + test_text
            )
        else:
...
def build_workflow(client: CopilotClient):
    ...
    workflow = (
        ...
        .add_edge(reviewer, implementer)  # loop back when review incomplete
        .build()
    )
    return workflow
